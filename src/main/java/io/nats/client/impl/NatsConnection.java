...
class NatsConnection implements Connection {
    ...
    void queueOutgoing(NatsMessage msg) {
        validateControlLineAndPayloadSizes(msg);
        if (!writer.queue(msg)) {
    ...
    private void validateControlLineAndPayloadSizes(NatsMessage msg) {
        ...
        if (options.clientSideLimitChecks()) {
            ...
            long maxPayload = this.getMaxPayload();
            if (payloadSize > maxPayload && maxPayload > 0) {
                throw new IllegalArgumentException(
                    "Message payload size exceeds server configuration " + payloadSize + " vs " + maxPayload);
            }
        }
    }
